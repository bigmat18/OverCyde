...
Texture3D::Texture3D(const std::vector<std::string> &paths, GLuint slot)
    : Texture(slot), filePaths(paths) {
  this->SetFlipVerticallyOnLoad(false);
  glGenTextures(1, &this->rendererID);
  glBindTexture(GL_TEXTURE_CUBE_MAP, this->rendererID);

  for (int i = 0; i < static_cast<int>(this->filePaths.size()); i++) {
    this->LoadImageBuffer(this->filePaths[i].c_str());
    if (this->localBuffer) {
      glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGBA, this->width,
